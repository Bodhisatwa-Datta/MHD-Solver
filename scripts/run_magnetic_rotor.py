"""Run the standard 2D fast magnetic-rotor benchmark."""
...
from mhd_solver.mhd2d.initial_conditions import magnetic_rotor
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--final-time", type=float, default=0.15)
    parser.add_argument("--figure", type=Path, default=Path("figures/magnetic_rotor.png"))
    return parser.parse_args()


def run(resolution: int, final_time: float):
    config = MHD2DConfig(
        nx=resolution,
        ny=resolution,
    ...
    return config, solve(config, magnetic_rotor)
...
def main() -> None:
    ...
    if args.comparison_resolution % args.resolution != 0:
        raise ValueError("comparison resolution must be an integer multiple")
    coarse_config, coarse = run(args.resolution, args.final_time)
    fine_config, fine = run(args.comparison_resolution, args.final_time)
    primitive = fine.primitive
    dx, dy = 1.0 / fine_config.nx, 1.0 / fine_config.ny
    density, vx, vy, _, pressure, bx, by, bz, _ = primitive
    magnetic_pressure = 0.5 * (bx**2 + by**2 + bz**2)
    current = current_density_z(primitive, dx, dy, periodic=False)
    ...
    stride = max(1, fine_config.nx // 24)
    axes[0, 2].streamplot(
        fine.x,
        fine.y,
        bx,
    ...
    figure.suptitle(
        f"Fast magnetic rotor at t = {fine.time:.2f} "
    ...
    record = {
        ...
        "integral_changes_with_outflow_boundaries": (
            fine.final_integrals - fine.initial_integrals
        ).tolist(),
        "divergence": {
            "final_L2": float(fine.divergence_l2[-1]),
            "maximum_L2": float(fine.divergence_l2.max()),
            "dimensionless_dx_L2_over_B_rms": float(
                dx * fine.divergence_l2[-1] / field_rms
            ),
        },
